...
class Base(object):
    ...
    def __init__(self, eqn_config):
        ...
        self.delta_t = self.total_time/self.t_grid_size
        self.y_init = None
...
class FBSDE(Base):
    ''' Multi-stock portfolio optimization with trading speed problem from <<<our paper ref>>> '''
    ...
    def ldx(self, t, x0, zeta):
        ...
        pi = TargDist()
        k = self.eqn_config.nu_support             # Support for target distribution
        ...
        p = [pi.p(k=k[i], T=(t/self.eqn_config.t_grid_size)) for i in range(len(k))]
        ...
        for i in range(len(k)):
            ...
            ldx += (1 / len(k)) * (tf.cast(-1 / (1 + tf.pow(np.e, (self.lamb*p[i] - np.mean(zeta[i])))), tf.float64)) * \
                   tf.cast(1/(1 + tf.pow(tf.cast(np.e, tf.float64), -self.lamb*(k[i] - x0))), tf.float64)
