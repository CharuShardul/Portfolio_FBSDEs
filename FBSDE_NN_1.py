...
class FBSDEsolver(object):
    def __init__(self, config, equation):
        ...
        self.y_init = self.model.y_init_0
        ...
        for p in range(self.fic_num):
            ...
            for step in range(self.net_config.num_iterations + 1):
                ...
                if step % self.net_config.logging_frequency == 0:
                    ...
                    training_history.append([p, step, loss, y_init, elapsed_time])
                    ...
                    logging.info("fict_num: %3u,   step: %5u,   loss: %.4e,  Y0: %.4e,   elapsed time: %3u"
                                 % (p, step, loss, y_init, elapsed_time))
            ...
            X0, Y0, Z0 = self.model(valid_data, training=False, train_end=True)
            print("Loss fn value at fict_num = ", p, " is = ", self.loss_fn(valid_data, training=False))
            trajec.append((X0, Y0, Z0))
            print("shapes=", X0.shape, Y0.shape, Z0.shape)
    ...
    def loss_fn(self, input, training):
        ...
        x0, y0 = self.model(input, training)
        #print("loss: y_shape", tf.shape(y0), tf.shape(y1))
        delta = tf.square(tf.abs(y0)) #beta*tf.reduce_sum(tf.stack([tf.square(tf.abs(y1[i]))
...
class fullNN(tf.keras.Model):
    def __init__(self, equation, config):
        ...
        #self.Exp_X = np.array([1.0 for _ in range(self.t_grid_size)])

        self.y_init_0 = tf.Variable(np.random.uniform(low=self.net_config.y_init_range[0],
                                                      high=self.net_config.y_init_range[1],
                                                      size=[1]))
        self.z_init_0 = tf.Variable(np.random.uniform(low=0.1,
                                                      high=0.2,
                                                      size=[1]))
    ...
    def call(self, input, training, train_end=False):
        ...
        if train_end==True:
            ...
            y_star_0 = y0
            z_star_0 = z0
            print("shape 0:", x_star_0.shape, y_star_0.shape, z_star_0.shape)

        for t in range(self.eqn_config.t_grid_size - 2):
            x0 = x0 + (W[:, t + 1, :] - W[:, t, :])
            #print("X0", x0.shape)
            y0 = y0 - (x0 - self.eqn_config.rho*y0) * dt + z0 * (W[:, t + 1, :] - W[:, t, :])
            #print("Y2", y0.shape)
            z = tf.concat([x0, y0], axis=-1)
            #print("z shape 0: ", tf.shape(z))
            z0 = self.subnet[t](z, training)
        ...
        x0 = x0 + (W[:, t + 2, :] - W[:, t+1, :])
        # print("X0", x0.shape)
        y0 = y0 - (x0 - self.eqn_config.rho*y0) * dt + z0 * (W[:, t + 2, :] - W[:, t+1, :])
        # print("Y2", y0.shape)
        if train_end == True:
            x_star_0 = tf.concat([x_star_0, x0], axis=-1)
            y_star_0 = tf.concat([y_star_0, y0], axis=-1)
        ...
        if train_end == False:
            ...
            return x_star_0.numpy()[:, :], y_star_0.numpy()[:, :], z_star_0.numpy()[:, :]
