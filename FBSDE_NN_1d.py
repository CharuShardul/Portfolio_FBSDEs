...
class FBSDEsolver(object):
    ...
    def train(self):
        ...
        for p in range(self.fic_num):
            ...
            #z_func = self.model.subnet
            X0, X1, Y0, Y1, Z0, Z1 = self.model(valid_data, training=False, train_end=True)
            print("Loss fn value at fict_num = ", p, " is = ", self.loss_fn(valid_data, training=False))
            print("shapes=", X0.shape, X1.shape, Y0.shape, Y1.shape, Z0.shape, Z1.shape)
            #self.model.Exp_X = np.mean(X0, axis=0)
            k = self.eqn_config.nu_support
            #print(np.log(1 + np.e**(self.lamb*(k[0] - X0[:, 2]))))
            self.model.zeta = np.array([[np.mean(np.log(1 + np.e**(self.lamb*(k[i] - X0[:, t]))))
                                         for i in range(len(k))]
                                        for t in range(self.eqn_config.t_grid_size)])
            trajec.append((X0, X1, Y0, Y1, Z0, Z1, self.model.zeta))
    ...
    def grad(self, input, training):
        with tf.GradientTape(persistent=True) as tape:
            loss = self.loss_fn(input, training=training)
...
class fullNN(tf.keras.Model):
    def __init__(self, equation, config):
        ...
        self.zeta = np.array([[np.log(1 + np.e**(self.lamb*(k-1))) for k in config.eqn_config.nu_support]
                              for _ in range(self.t_grid_size)])

        self.y_init_0 = tf.Variable(np.random.uniform(low=config.net_config.y_init_range[0],
    ...
    def call(self, input, training, train_end=False):
        ...
        if train_end==True:
            ...
            z_star_0 = z0
            z_star_1 = z1
            print("shape 0:", x_star_0.shape, x_star_1.shape, y_star_0.shape, y_star_1.shape, z_star_0.shape, z_star_1.shape)
        ...
        for t in range(self.eqn_config['t_grid_size'] - 2):
            ...
            #print("Y1", y0.shape)
            y0 = y0 - self.equation.f(t, x, y, z, S[:, t, None], self.zeta[t])[0] * dt + z0 * (
                        W[:, t + 1, :] - W[:, t, :])
            #print("Y2", y0.shape)
            y1 = y1 - self.equation.f(t, x, y, z, S[:, t, None], self.zeta[t])[1] * dt + \
                 z1 * (W[:, t + 1, :] - W[:, t, :])
            # print('y1_shape', tf.shape(y1), tf.shape(y1[1]))
            x1 = x1 - 200*y1 * dt #+ epsilon * (W[:, t + 1, :] - W[:, t, :])
            # print('x1shape', np.shape(x1))
            y = (y0, y1)
            z = tf.concat([x0, x1, y0, y1], axis=-1)
            #print("z shape 0: ", tf.shape(z))
            z = self.subnet[t](z, training=training)
        ...
        x = (x0, x1)  # tf.stack((x0, x1), axis=1)
        # print("Y1", y0.shape)
        y0 = y0 - self.equation.f(t+1, x, y, z, S[:, t+1, None], self.zeta[t+1])[0] * dt + z0 * (
                W[:, t+2, :] - W[:, t+1, :])
        # print("Y2", y0.shape)
        y1 = y1 - self.equation.f(t+1, x, y, z, S[:, t+1, None], self.zeta[t+1])[1] * dt + \
             z1 * (W[:, t+2, :] - W[:, t+1, :])
        # print('y1_shape', tf.shape(y1), tf.shape(y1[1]))
        x1 = x1 - 200*y1 * dt #+ epsilon * (W[:, t + 1, :] - W[:, t, :])
        if train_end == True:
...
class Subnet(tf.keras.Model):
    def __init__(self, config):
        ...
        Z_dim = dim_0 + dim_1
        num_hiddens = [20 + config.eqn_config.X_dim, 20 + config.eqn_config.X_dim]
        self.bn_layers = [
    ...
    def call(self, z, training):
        ...
        z = self.bn_layers[-1](z, training=training)
